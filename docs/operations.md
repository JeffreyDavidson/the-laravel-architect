...
When a release introduces responsive project images, run `php artisan projects:generate-image-variants` once after the persistent public-media directory is mounted. The command preserves original uploads, creates WebP derivatives beside them, and returns a failure if a source file is missing or unsupported. Do not remove the original images.
