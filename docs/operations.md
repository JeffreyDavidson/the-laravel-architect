...
The command fails when the checked-out commit differs, migrations are pending, SQLite integrity or effective connection safeguards fail, the Nightwatch agent is unavailable, queue or scheduler heartbeats are stale, any configured backup disk lacks a fresh backup, or stored media lacks a required responsive variant. Then verify all of the following against the deployed commit:
