...
@section('content')
    <x-hero-section>
        <div class="grid gap-8 lg:grid-cols-[8rem_minmax(0,1fr)] lg:gap-10">
            <p class="text-brand-600 font-mono text-xs tracking-label uppercase">Audio / 03</p>
            <div>
                ...
                <p class="mt-6 max-w-2xl text-lg leading-relaxed text-gray-600 dark:text-gray-400">
                    Coffee with The Laravel Architect is where Laravel, architecture, and the developer life meet.
                </p>
                <div class="mt-7 flex gap-6 font-mono text-xs tracking-wide text-gray-500 uppercase">
        ...
        <div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 md:py-20 lg:px-8">
            @if ($podcast)
                <a
                    ...
                    class="group focus-visible:outline-brand-400 grid gap-10 border-y border-gray-200 py-10 focus-visible:outline-2 focus-visible:outline-offset-4 md:grid-cols-[18rem_minmax(0,1fr)] md:items-center md:py-14 dark:border-surface-border"
                >
                    @if ($podcast->cover_image_url)
                        ...
                        />
                    @else
                        <div class="flex aspect-square w-full max-w-72 items-center justify-center border border-surface-border bg-surface-card">
                            <x-svg-icon name="microphone" class="text-brand-600 h-12 w-12" />
                        </div>
                    @endif

                    <div>
                        <p class="text-brand-600 font-mono text-xs tracking-label uppercase">Current show</p>
                        ...
                        <ul
                            aria-label="Topics"
                            class="mt-7 flex flex-wrap gap-x-5 gap-y-2 font-mono text-xs tracking-wide text-gray-500 uppercase"
                        >
                            @foreach (['Laravel', 'Architecture', 'Testing', 'Career', 'Guest interviews'] as $topic)
                                <li>{{ $topic }}</li>
                            @endforeach
                        </ul>
                        <span class="text-brand-600 mt-8 inline-flex items-center gap-2 text-sm font-semibold"
                            >View show
                            <x-svg-icon
                ...
                <section aria-labelledby="podcast-format-heading" class="mt-20">
                    <div class="mb-8 grid gap-3 border-b border-gray-200 pb-5 md:grid-cols-[10rem_1fr] dark:border-surface-border">
                        <p class="text-brand-600 font-mono text-xs tracking-label uppercase">Format</p>
                    ...
                    <div class="grid md:grid-cols-3">
                        @foreach ([
                            ...
                            <article class="border-b border-gray-200 py-7 md:border-r md:border-b-0 md:px-7 md:first:pl-0 md:last:border-r-0 dark:border-surface-border">
            ...
            @else
                <div class="border-y border-gray-200 py-20 text-center dark:border-surface-border">
                    <p class="font-mono text-xs tracking-label text-gray-500 uppercase">Recording in progress</p>
