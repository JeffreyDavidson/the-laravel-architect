...
<section class="noise-overlay relative overflow-hidden border-b border-[#1e2a3a]">
    ...
    <div class="absolute top-1/2 left-1/3 w-[600px] h-[600px] rounded-full opacity-[0.07] blur-[120px]" style="background: radial-gradient(circle, {{ $podcast->color }}, transparent 70%);"></div>
    <div class="absolute bottom-0 right-1/4 w-[400px] h-[400px] rounded-full opacity-[0.05] blur-[100px]" style="background: radial-gradient(circle, {{ $podcast->color }}, transparent 70%);"></div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        <div class="flex flex-col lg:flex-row gap-12">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="mb-10 rounded-2xl border border-[#1e2a3a] bg-[#0D1117] overflow-hidden" style="box-shadow: 0 0 60px {{ $podcast->color }}08;"
                     x-data="{
                        audio: null,
                        playing: false,
                        currentTime: 0,
                    ...
                    <div class="px-6 pb-6 flex items-center justify-between">
                        ...
                        {{-- Speed control --}}
                        <div x-data="{ speed: 1 }">
                            <button @click="speed = speed >= 2 ? 0.5 : speed + 0.25; $refs.audio.playbackRate = speed" class="px-2.5 py-1 rounded-lg border border-[#1e2a3a] text-xs font-mono text-gray-400 hover:text-white hover:border-gray-600 transition-colors">
        ...
        <div class="mt-16 pt-8 border-t border-[#1e2a3a]">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @if($prevEpisode)
                <a href="{{ route('podcast.episode', [$podcast, $prevEpisode]) }}" class="ep-nav group p-5 rounded-2xl border border-[#1e2a3a] transition-all duration-300 hover:bg-[#0D1117]">
                    <div class="flex items-center gap-3">
                ...
                @if($nextEpisode)
                <a href="{{ route('podcast.episode', [$podcast, $nextEpisode]) }}" class="ep-nav group p-5 rounded-2xl border border-[#1e2a3a] transition-all duration-300 hover:bg-[#0D1117] text-right">
