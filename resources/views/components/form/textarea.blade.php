@props([
    'id',
    'name',
    'rows' => 6,
    'required' => false,
    'placeholder' => null,
])

@php($hasError = $errors->has($name))

<textarea
    id="{{ $id }}"
    name="{{ $name }}"
    rows="{{ $rows }}"
    @required($required)
    @if ($hasError) aria-invalid="true" aria-describedby="{{ $id }}-error" @endif
    @if ($placeholder) placeholder="{{ $placeholder }}" @endif
    {{
        $attributes->class([
            'w-full rounded-xl border bg-white px-4 py-3 text-base text-gray-900 outline-none transition-[border-color,box-shadow] placeholder:text-gray-600 focus:ring-2 dark:bg-surface-control dark:text-gray-200 sm:text-sm',
            'border-red-500 focus:border-red-500 focus:ring-red-500/10' => $hasError,
            'border-gray-300 focus:border-brand-600 focus:ring-brand-600/10 dark:border-surface-border' => ! $hasError,
        ])
    }}
>{{ old($name, trim($slot)) }}</textarea>
@error($name)
    <p id="{{ $id }}-error" class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
@enderror
