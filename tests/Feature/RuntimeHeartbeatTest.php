...
it('schedules a heartbeat that records the scheduler and probes the queue', function () {
    ...
    expect(Cache::get(RuntimeHealth::SCHEDULER_HEARTBEAT_KEY))->toBe(now()->getTimestamp());
...
it('records a heartbeat when the queued probe is processed', function () {
    ...
    app(RecordQueueHeartbeat::class)
        ->handle(app(RuntimeHealth::class));

    expect(Cache::get(RuntimeHealth::QUEUE_HEARTBEAT_KEY))->toBe(now()->getTimestamp());
});
