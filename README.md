...
Production observability is provided by Laravel Nightwatch. It is disabled by default for local development and tests, samples 10% of web requests, and does not capture request payloads, request headers, exception source code, mail events, authenticated user names, authenticated user email addresses, visitor IP addresses, complete request URLs, Artisan command arguments, or raw cache keys. Request telemetry retains Laravel route templates while removing route parameters and query strings so signed newsletter URLs and confirmation tokens cannot be ingested. Outgoing-request telemetry removes query strings and fragments so API credentials, YouTube channel identifiers, and video identifiers are not ingested. Command telemetry retains only the registered command name. Cache telemetry replaces keys with application-keyed HMAC digests, preserving per-key grouping without exposing the source value. Database exception messages and previews are replaced before ingestion because Laravel includes bound values in query failures. Mail-event capture remains disabled because contact-mail subjects contain visitor names. Connect the site through Forge's built-in Nightwatch integration, keep its environment token only in Forge, and set `NIGHTWATCH_ENABLED=true` in production. See [`docs/operations.md`](docs/operations.md) for setup and verification.
