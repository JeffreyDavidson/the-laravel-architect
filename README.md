...
Production observability is provided by Laravel Nightwatch. It is disabled by default for local development and tests, samples 10% of web requests, and does not capture request payloads, request headers, exception source code or messages, mail events, application log messages or context, authenticated user names, authenticated user email addresses or raw identifiers, visitor IP addresses, complete request URLs, Artisan command arguments, raw cache keys, or SQL literal values and comments. Request telemetry retains Laravel route templates while removing route parameters and query strings so signed newsletter URLs and confirmation tokens cannot be ingested. Outgoing-request telemetry removes query strings and fragments so API credentials, YouTube channel identifiers, and video identifiers are not ingested. Authenticated user IDs and cache keys are replaced with application-keyed HMAC digests, preserving correlation without exposing source values. Command telemetry retains only the registered command name. Query telemetry retains SQL structure and placeholders while replacing raw literals and comments. Exception telemetry retains class, code, location, and its type-only stack trace while replacing free-form messages and previews. Mail-event and application-log capture remain disabled because their free-form content can contain personal or secret values. Connect the site through Forge's built-in Nightwatch integration, keep its environment token only in Forge, and set `NIGHTWATCH_ENABLED=true` in production. See [`docs/operations.md`](docs/operations.md) for setup and verification.
