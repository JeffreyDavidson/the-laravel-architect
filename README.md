...
Public pages are server-rendered Blade views. Controllers pass page-specific `SEOData` or an SEO-enabled content model to the shared layout, which renders titles, descriptions, canonical links, social metadata, and robots directives. The shared JSON-LD graph uses named Laravel routes for canonical site, author, static-page, article, podcast, episode, project case-study, collection, item-list, and breadcrumb entities. Route-model binding uses content slugs, while publication scopes keep drafts and future content off public pages, feeds, and the sitemap. Signed newsletter action pages are explicitly excluded from indexing.
