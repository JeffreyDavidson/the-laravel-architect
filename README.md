...
Uploaded images and audio are validated and stored through Laravel's `public` filesystem disk. Models store explicit file paths and remove replaced or record-owned files; deleting a podcast also removes media owned by its database-cascaded episodes. Project images retain their original upload as the canonical fallback and generate 640px and 1280px WebP variants for responsive public rendering. Replacing or deleting a project image also removes its variants. Run `php artisan storage:link` on a new environment and `php artisan projects:generate-image-variants` once when backfilling existing project images.
