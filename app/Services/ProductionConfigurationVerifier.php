...
final class ProductionConfigurationVerifier
{
    private const array REQUIRED_NIGHTWATCH_PAYLOAD_REDACTIONS = [
        '_token', 'password', 'password_confirmation', 'email', 'name', 'message',
    ];
    ...
    private function includesAppHostname(mixed $allowedHostnames, mixed $appUrl): bool
    {
        ...
        $normalizedHostnames = array_map(
            fn (mixed $hostname): string => is_string($hostname) ? strtolower(rtrim(trim($hostname), '.')) : '',
            $allowedHostnames,
        );
    ...
    private function hasSafeBackupFileSources(mixed $sources, mixed $excludes): bool
    {
        ...
        return ! $sourceOverlapsRelease && in_array(base_path('.env'), $excludedPaths, true);
    }
    ...
    private function hasConfiguredB2Backup(mixed $disks): bool
    {
        ...
        $endpoint = is_array($disk) ? ($disk['endpoint'] ?? null) : null;
    ...
    {
        return is_float($sampleRate) && $sampleRate > 0 && $sampleRate <= $maximum;
    }
    ...
    private function containsRequiredNightwatchRedactions(mixed $configured, array $required): bool
    {
        ...
        return array_all($required, fn (string $value): bool => in_array(strtolower($value), $normalized, true));
    }
